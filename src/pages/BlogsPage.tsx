...
const POSTS = [
  { cat: 'Technology', title: 'Building a digital backbone across a multi-business group', date: 'Jun 2026', read: '6 min', img: 'photo-1498050108023-c5249f4df085', feat: true,
    excerpt: 'How shared infrastructure lets independent companies move faster without losing their edge.' },
    ...
    <div style={{ overflowX: 'hidden', background: '#fff' }}>
      ...
      <section style={{ background: 'linear-gradient(180deg,#ffffff,#f3faf7)', padding: 'clamp(48px,6vw,90px) clamp(24px,5vw,80px)' }}>
        <div style={{ maxWidth: 1760, margin: '0 auto' }}>
          <motion.a href="/contact" className="bl-feat"
            ...
            <div className="bl-feat-body">
              ...
              <span className="bl-feat-link">Read article <ArrowUpRight size={16} strokeWidth={2.4} /></span>
            </div>
          </motion.a>
        </div>
      </section>
      ...
      <section style={{ background: '#fff', padding: 'clamp(48px,6vw,90px) clamp(24px,5vw,80px) clamp(64px,8vw,120px)' }}>
        <div style={{ maxWidth: 1760, margin: '0 auto' }}>
          ...
          <div className="bl-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 20, marginTop: 36 }}>
            {rest.map((p, i) => (
              <motion.a key={p.title} href="/contact" className="bl-card"
                ...
                <div className="bl-card-body">
                  ...
                  <div className="bl-card-foot"><span className="bl-meta">{p.date} · {p.read}</span><span className="bl-card-arrow"><ArrowUpRight size={16} strokeWidth={2.4} /></span></div>
                </div>
              </motion.a>
            ))}
          </div>
      ...
      <style>{`
        ...
        .bl-feat-ex { font-family:'Inter',sans-serif; font-size:clamp(14px,1.1vw,16px); color:${MUTED}; line-height:1.7; margin:0 0 20px; }
        .bl-feat-link { display:inline-flex; align-items:center; gap:8px; font-family:'Inter',sans-serif; font-size:13px; font-weight:700; letter-spacing:1px; text-transform:uppercase; color:${GREEN}; transition: gap 0.3s ease; }
        .bl-feat:hover .bl-feat-link { gap:14px; }
        ...
        .bl-card-title { font-family:'Poppins',sans-serif; font-weight:600; font-size:clamp(17px,1.4vw,20px); color:${NAVY}; letter-spacing:-0.01em; line-height:1.3; margin:0 0 16px; }
        .bl-card-foot { display:flex; align-items:center; justify-content:space-between; margin-top:auto; }
        .bl-card-arrow { display:inline-flex; color:${NAVY}; opacity:0.4; transition: transform 0.4s ${'cubic-bezier(0.16,1,0.3,1)'}, color 0.3s ease, opacity 0.3s ease; }
        .bl-card:hover .bl-card-arrow { color:${GREEN}; opacity:1; transform:translate(3px,-3px); }
